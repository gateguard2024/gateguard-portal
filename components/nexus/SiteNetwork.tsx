...
// Per-site UniFi — cloud-only (Site Manager API). Internet/WAN status, client mix,
// and device/gear health, read-only. Local-controller paths were removed: the portal
// is cloud-hosted and can't reach an on-prem 192.168.x.x console, so they only added
// confusion. Dealer-facing: when nothing is connected, the widget hides itself
// (corporate still sees the setup prompt).
...
export function SiteNetwork({ siteId }: { siteId: string }) {
  ...
  const load = React.useCallback(() => {
    setLoading(true);
    fetch(`/api/unifi/cloud/overview?site_id=${siteId}`)
      .then(async r => ({ ok: r.ok, d: await r.json().catch(() => ({})) }))
      .then(o => setOv(o.ok && o.d?.connected ? o.d : null))
      .catch(() => {})
