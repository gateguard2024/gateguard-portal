...
const STATUS_COLOR: Record<string, string> = { open: '#5FB8E0', scheduled: '#fbbf24', in_progress: '#9FD8EC', new: '#8FD3EC', on_hold: '#f87171', assigned: '#7ee0a8' }
...
function Kpi({ glyph, value, label, sub, subColor, foot }: { glyph: string; value: string | number; label: string; sub: string; subColor: string; foot?: string }) {
  return (
    <div className="rounded-2xl p-3.5" style={TILE_STYLE}>
      ...
      {foot && <div className="mt-0.5 text-[9px]" style={{ color: '#9FD8EC' }}>{foot}</div>}
    </div>
  )
}
    ...
    <div className="flex flex-col gap-1.5">
      {funnel.map(f => (
        <div key={f.key} className="flex items-center gap-2">
          ...
          {fin && <div className="w-12 shrink-0 text-right text-[10px]" style={{ color: '#9FD8EC' }}>{money(f.value)}</div>}
        </div>
      ))}
    </div>
  )
...
function FleetDonut({ health }: { health: Fleet }) {
  ...
  const segs = [
    ...
    { v: health.offline, c: '#f87171', label: 'Offline' },
  ]
  ...
  return (
    <div className="flex items-center gap-4">
      <svg width="92" height="92" viewBox="0 0 92 92" className="shrink-0">
        ...
        <text x="46" y="57" textAnchor="middle" fontSize="8" fill="#c3d3e2">online</text>
      </svg>
      <div className="flex flex-col gap-1.5">
        {segs.map((s, i) => (
...
function PeopleTable({ people, fin, loading }: { people: Rollup['people']; fin: boolean; loading: boolean }) {
  if (loading) return <div className="py-6 text-center text-[12px]" style={{ color: '#9FD8EC' }}>Loading team…</div>
  if (people.length === 0) return <Empty label="No assigned records in your hierarchy yet" />
  return (
    <div className="max-h-56 overflow-y-auto">
      <table className="w-full text-[12px]">
        <thead>
          <tr className="text-[9.5px] uppercase tracking-[0.06em]" style={{ color: '#9FD8EC' }}>
        ...
        <tbody>
          {people.map((p, i) => (
            <tr key={i} style={{ borderTop: '1px solid rgba(140,170,200,0.1)' }}>
              ...
              <td className="py-1.5 pr-2" style={{ color: '#c3d3e2' }}>{p.org}</td>
...
function Empty({ label }: { label: string }) {
  return <div className="rounded-xl px-3 py-6 text-center text-[11px]" style={{ color: '#9FD8EC' }}>{label}</div>
}
