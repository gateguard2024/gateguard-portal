...
function ActionButton({ label, onClick, muted }: { label: string; onClick?: () => void; muted?: boolean }) {
  ...
  return (
    <button
      ...
      style={muted
        ? { background: 'linear-gradient(135deg, rgba(255,255,255,0.08), rgba(95,184,224,0.055))', border: '1px solid rgba(255,255,255,0.22)', color: 'rgba(255,255,255,0.92)', boxShadow: '0 0 16px rgba(95,184,224,0.08), inset 0 1px 0 rgba(255,255,255,0.08)' }
...
function SimpleOverview({ detail, onOpen }: { detail: SimpleDetail; onOpen: (href: string) => void }) {
  return (
    <div className="mt-4 rounded-3xl p-4" style={{ background: 'rgba(95,184,224,0.075)', border: '1px solid rgba(95,184,224,0.20)', boxShadow: '0 0 28px rgba(95,184,224,0.10)' }}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="text-[10px] uppercase tracking-[0.18em]" style={{ color: '#9FD8EC' }}>Simple Overview</div>
        ...
        <div className="rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em]" style={{ background: 'rgba(95,184,224,0.10)', border: '1px solid rgba(95,184,224,0.20)', color: '#9FD8EC', whiteSpace: 'nowrap' }}>
      ...
      <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
        {detail.details.length > 0 ? detail.details.map(item => (
          <div key={`${item.label}-${item.value}`} className="rounded-2xl px-3 py-2" style={{ background: 'rgba(0,0,0,0.18)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="text-[9px] uppercase tracking-[0.14em]" style={{ color: 'rgba(255,255,255,0.82)' }}>{item.label}</div>
            <div className="mt-1 text-xs" style={{ color: 'rgba(255,255,255,0.78)' }}>{item.value}</div>
          </div>
        )) : (
          <div className="rounded-2xl px-3 py-2 text-xs" style={{ background: 'rgba(0,0,0,0.18)', border: '1px solid rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.82)' }}>No extra details yet.</div>
        )}
      </div>

      <div className="mt-4 flex flex-wrap gap-2">
        {detail.actions.map(action => (
          <button key={`${action.label}-${action.href}`} type="button" onClick={() => onOpen(action.href)} className="rounded-full px-3 py-1.5 text-[11px] font-semibold" style={{ background: action.label.startsWith('Open') ? 'linear-gradient(135deg, #5FB8E0, #5FB8E0)' : 'rgba(255,255,255,0.05)', border: action.label.startsWith('Open') ? '1px solid rgba(95,184,224,0.35)' : '1px solid rgba(255,255,255,0.08)', color: 'white' }}>
            {action.label}
          </button>
        ))}
      </div>
    ...
    <div className="rounded-3xl p-4" style={{ background: 'linear-gradient(180deg, rgba(8,18,34,0.70), rgba(3,9,22,0.48))', border: '1px solid rgba(59,130,246,0.16)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05)' }}>
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row">
        <input
          ...
          style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.18)', color: 'rgba(255,255,255,0.88)' }}
        />
        <button type="button" onClick={() => void runSearch()} disabled={loading} className="rounded-2xl px-4 py-3 text-sm font-semibold disabled:opacity-50" style={{ background: 'linear-gradient(135deg, #5FB8E0, #5FB8E0)', color: 'white', boxShadow: '0 0 20px rgba(95,184,224,0.18)' }}>
          {loading ? 'Searching…' : 'Search'}
        </button>
      </div>

      {message && <div className="mt-3 rounded-2xl px-3 py-2 text-[11px]" style={{ background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.07)', color: 'rgba(255,255,255,0.82)' }}>{message}</div>}
          ...
          {results.map(result => {
            ...
            return (
              <button
                ...
                style={{
                  background: isSelected ? 'rgba(95,184,224,0.12)' : 'rgba(0,0,0,0.18)',
                  border: isSelected ? '1px solid rgba(95,184,224,0.34)' : '1px solid rgba(255,255,255,0.06)',
                }}
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold" style={{ color: 'rgba(255,255,255,0.9)' }}>{result.title}</div>
                    <div className="mt-1 text-[11px]" style={{ color: 'rgba(255,255,255,0.82)' }}>{result.subtitle}</div>
                    {result.meta && <div className="mt-1 text-[10px]" style={{ color: 'rgba(255,255,255,0.82)' }}>{result.meta}</div>}
                  </div>
                  <div className="rounded-full px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.12em]" style={{ background: 'rgba(95,184,224,0.10)', border: '1px solid rgba(95,184,224,0.18)', color: '#9FD8EC', whiteSpace: 'nowrap' }}>{typeLabel(result.type)}</div>
                </div>
              </button>
            )
          })}
      ...
      {selected && (
        <div className="mt-4 rounded-3xl p-4" style={{ background: 'rgba(95,184,224,0.07)', border: '1px solid rgba(95,184,224,0.18)' }}>
          <div className="text-[10px] uppercase tracking-[0.18em]" style={{ color: '#9FD8EC' }}>Selected</div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {(selected.type === 'site' || selected.type === 'property') && (
              <button type="button" onClick={() => setManageSiteId(selected.id)} className="rounded-full px-3 py-1.5 text-[11px] font-semibold" style={{ background: 'linear-gradient(135deg, #5FB8E0, #5FB8E0)', color: 'white' }}>Open site &amp; systems →</button>
            )}
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/68 px-4 py-6 backdrop-blur-sm">
      <div
        ...
        style={{
          background: 'radial-gradient(circle at 18% 0%, rgba(95,184,224,0.16), transparent 32%), linear-gradient(180deg, rgba(8,18,34,0.97), rgba(3,9,22,0.97))',
          border: '1px solid rgba(95,184,224,0.20)',
          boxShadow: '0 30px 100px rgba(0,0,0,0.60), 0 0 58px rgba(95,184,224,0.13), inset 0 1px 0 rgba(255,255,255,0.07)',
      ...
      >
        <div className="min-h-0 overflow-y-auto pr-1" style={{ overscrollBehavior: 'contain', WebkitOverflowScrolling: 'touch' }}>
          ...
          <div className="text-[10px] uppercase tracking-[0.24em]" style={{ color: 'rgba(95,184,224,0.82)' }}>Operations</div>
          <h2 className="mt-1 text-2xl font-semibold" style={{ color: 'rgba(255,255,255,0.97)', textShadow: '0 0 18px rgba(95,184,224,0.20)' }}>{title}</h2>
...
export function CustomersSitesSurface() {
  ...
  const cards: CustomersSitesCard[] = [
    { id: 'find-customer', title: 'Find Customer', subtitle: 'Look up a customer account, contact, billing relationship, or recent work.', hex: '#5FB8E0', glyph: 'lead' },
    { id: 'find-property', title: 'Find Property', subtitle: 'Search by property name, address, site, gate, or management company.', hex: '#5FB8E0', glyph: 'research' },
  ...
  return (
    <section className="mt-9 w-full max-w-5xl">
      <div className="rounded-[2rem] p-5 sm:p-6" style={{ background: 'radial-gradient(circle at 12% 0%, rgba(95,184,224,0.16), transparent 34%), linear-gradient(180deg, rgba(8,18,34,0.78), rgba(3,9,22,0.72))', border: '1px solid rgba(95,184,224,0.18)', boxShadow: '0 28px 90px rgba(0,0,0,0.38), 0 0 46px rgba(95,184,224,0.12), inset 0 1px 0 rgba(255,255,255,0.07)', backdropFilter: 'blur(26px)' }}>
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="text-[10px] uppercase tracking-[0.24em]" style={{ color: 'rgba(95,184,224,0.82)' }}>Operations</div><h2 className="mt-1 text-xl font-semibold leading-tight" style={{ color: 'rgba(255,255,255,0.97)', textShadow: '0 0 18px rgba(95,184,224,0.22)' }}>Who or what property are we working on?</h2><p className="mt-1 max-w-2xl text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.54)' }}>Find the customer, property, site systems, or anything that needs attention.</p></div><div className="rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.18em]" style={{ background: 'rgba(95,184,224,0.14)', color: 'rgba(125,229,255,0.96)', border: '1px solid rgba(95,184,224,0.28)', boxShadow: '0 0 18px rgba(95,184,224,0.12)' }}>Site OS</div></div>
        ...
        <div className="mt-5 text-[11px]" style={{ color: 'rgba(255,255,255,0.82)' }}>Operations stays simple: find the person, find the property, review what needs attention, or open the site systems.</div>
      </div>
