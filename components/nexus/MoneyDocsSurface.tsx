...
type MoneyDocsCard = {
  id: Exclude<MoneyDocsPanel, null>
  title: string
  subtitle: string
  hex: string
...
function MoneyDocsCardButton({ card, onClick }: { card: MoneyDocsCard; onClick: () => void }) {
...
function MoneyDocsDetailShell({ title, subtitle, onClose, children, actions }: { title: string; subtitle: string; onClose: () => void; children: React.ReactNode; actions?: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/68 px-4 py-6 backdrop-blur-sm">
      <div
        ...
        style={{
          background: 'radial-gradient(circle at 18% 0%, rgba(251,191,36,0.14), transparent 32%), linear-gradient(180deg, rgba(8,18,34,0.97), rgba(3,9,22,0.97))',
          border: '1px solid rgba(251,191,36,0.18)',
          boxShadow: '0 30px 100px rgba(0,0,0,0.60), 0 0 58px rgba(251,191,36,0.10), inset 0 1px 0 rgba(255,255,255,0.07)',
...
function ComingSoonPanel({ copy }: { copy: string }) {
  return (
    <div className="rounded-3xl p-4" style={{ background: 'linear-gradient(180deg, rgba(8,18,34,0.70), rgba(3,9,22,0.48))', border: '1px solid rgba(251,191,36,0.16)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05)' }}>
      <div className="text-sm font-semibold" style={{ color: 'rgba(255,255,255,0.94)' }}>Board coming next</div>
      <p className="mt-2 text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.54)' }}>{copy}</p>
    </div>
  )
}

export function MoneyDocsSurface() {
  ...
  return (
    <section className="mt-9 w-full max-w-5xl">
      <div
        ...
        style={{
          background: 'radial-gradient(circle at 12% 0%, rgba(251,191,36,0.13), transparent 34%), linear-gradient(180deg, rgba(8,18,34,0.78), rgba(3,9,22,0.72))',
          border: '1px solid rgba(251,191,36,0.16)',
          boxShadow: '0 28px 90px rgba(0,0,0,0.38), 0 0 46px rgba(251,191,36,0.09), inset 0 1px 0 rgba(255,255,255,0.07)',
      ...
      >
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="text-[10px] uppercase tracking-[0.24em]" style={{ color: 'rgba(251,191,36,0.82)' }}>Money/Docs</div>
            <h2 className="mt-1 text-xl font-semibold leading-tight" style={{ color: 'rgba(255,255,255,0.97)', textShadow: '0 0 18px rgba(251,191,36,0.16)' }}>What money or paperwork needs attention?</h2>
            <p className="mt-1 max-w-2xl text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.54)' }}>
              Invoices, renewals, signatures, and compliance items all start here.
            </p>
          </div>
          <div className="rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.18em]" style={{ background: 'rgba(251,191,36,0.12)', color: 'rgba(253,230,138,0.96)', border: '1px solid rgba(251,191,36,0.26)', boxShadow: '0 0 18px rgba(251,191,36,0.10)' }}>Back Office</div>
        </div>
