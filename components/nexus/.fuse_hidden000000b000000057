          ...
          : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px,1fr))', gap: 12 }}>
              {shown.map(s => (
                <button key={s.id} onClick={() => setOpenSite(s)} style={card}>
                  ...
                  {(s.address || s.city) && <div style={{ fontSize: 12, color: '#98abbd', marginTop: 4 }}>{[s.address, s.city, s.state].filter(Boolean).join(', ')}</div>}
                  <div style={{ fontSize: 11.5, color: '#7f96ab', marginTop: 8 }}>{s.units ? `${s.units} units · ` : ''}Tap to open systems →</div>
                </button>
              ))}
            </div>}
      </div>
