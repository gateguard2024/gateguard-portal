...
const PRIORITY_COLOR: Record<string, string> = { high: '#f87171', normal: '#9FD8EC', low: '#94a3b8' }
  ...
  return (
    <div className="space-y-3">
          ...
          {FILTERS.map(f => {
            ...
            return (
              <button key={f.id} type="button" onClick={() => { setFilter(f.id); setOpenId(null); setSelectedId(null) }}
                className={`rounded-lg px-3 py-1 text-[11px] font-semibold transition-all ${active ? 'border border-[#5FB8E0]/45 bg-[#2f7fb8]/30 text-[#bfe6ff]' : 'text-slate-200 hover:text-slate-200'}`}>
                {f.label}
              </button>
            )
          })}
      ...
      <div className="flex gap-2">
        ...
        <button type="button" disabled={busy || !quick.trim()} onClick={() => void quickAdd()} className="rounded-xl px-4 py-2 text-xs font-bold disabled:opacity-40" style={{ background: 'rgba(95,184,224,0.16)', border: '1px solid rgba(95,184,224,0.4)', color: '#9FD8EC' }}>Add</button>
      </div>
