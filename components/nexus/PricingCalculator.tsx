...
const tagS = { fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: '#9FD8EC' } as const
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', fontSize: 13, fontWeight: 600, marginBottom: 4 }}>
        <span style={{ color: '#eaf2fb' }}>{label}{hint ? <span style={{ ...tagS, marginLeft: 6 }}>{hint}</span> : null}</span>
    ...
    <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
      {options.map(o => (
        <button key={o.v} type="button" onClick={() => onChange(o.v)}
          style={{ flex: 1, minWidth: 54, padding: '6px', borderRadius: 9, fontSize: 11, fontWeight: 600, cursor: 'pointer', ...(value === o.v ? { background: '#5FB8E0', border: '1px solid #5FB8E0', color: '#fff' } : { background: 'transparent', border: '1px solid rgba(255,255,255,0.18)', color: '#c3d3e2' }) }}>{o.label}</button>
      ))}
    </div>
  )
}
...
function Row({ label, total, units, strong }: { label: string; total: number; units: number; strong?: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderTop: '1px solid rgba(255,255,255,0.08)', fontVariantNumeric: 'tabular-nums' }}>
      <span style={{ color: '#eaf2fb', fontWeight: strong ? 800 : 400 }}>{label}</span>
      <span style={{ display: 'flex', gap: 14 }}>
        <span style={{ width: 62, textAlign: 'right', color: '#9FD8EC' }}>{usd(pu)}</span>
        <span style={{ width: 78, textAlign: 'right', color: '#eaf2fb', fontWeight: strong ? 800 : 400 }}>{usd(total)}</span>
      </span>
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 rounded-full p-1 text-[12px] font-semibold" style={{ background: 'linear-gradient(180deg,#1b2836,#141e29)', border: '1px solid rgba(140,170,200,0.2)', width: 'fit-content' }}>
        <button type="button" onClick={() => setTab('monthly')} className="rounded-full px-3.5 py-1.5" style={tab === 'monthly' ? { background: 'rgba(95,184,224,0.25)', border: '1px solid rgba(95,184,224,0.5)', color: '#9FD8EC' } : { color: '#c3d3e2' }}>Monthly recurring</button>
        <button type="button" onClick={() => setTab('install')} className="rounded-full px-3.5 py-1.5" style={tab === 'install' ? { background: 'rgba(95,184,224,0.25)', border: '1px solid rgba(95,184,224,0.5)', color: '#9FD8EC' } : { color: '#c3d3e2' }}>One-time install</button>
      </div>
      ...
      <div style={card}>
        <div className="grid grid-cols-1 gap-x-5 sm:grid-cols-2">
          ...
          <div>
            ...
            <label className="flex cursor-pointer items-center gap-2 text-[12px]" style={{ color: '#c3d3e2', marginTop: 4 }}>
            ...
            {naturalBand !== 'dealer' && (
              <div className="mt-3 flex items-center gap-1 rounded-full p-1 text-[11px] font-semibold" style={{ background: 'linear-gradient(180deg,#1b2836,#141e29)', border: '1px solid rgba(140,170,200,0.2)', width: 'fit-content' }}>
                {(naturalBand === 'corporate' ? ['corporate', 'distributor', 'dealer'] : ['distributor', 'dealer']).map(b => (
                  <button key={b} type="button" onClick={() => setPreview(b === naturalBand ? '' : (b as 'corporate' | 'distributor' | 'dealer'))} className="rounded-full px-3 py-1" style={band === b ? { background: 'rgba(95,184,224,0.2)', border: '1px solid rgba(95,184,224,0.5)', color: '#9FD8EC' } : { color: '#c3d3e2' }}>{b === 'corporate' ? 'Corporate' : b === 'distributor' ? 'Distributor' : 'Dealer'}</button>
                ))}
              </div>
            )}
      ...
      <div className="text-[11px]" style={{ color: '#9FD8EC' }}>
        {showInternal
