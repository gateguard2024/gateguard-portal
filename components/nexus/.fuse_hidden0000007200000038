...
export function SiteNetworkDevices({ siteId }: { siteId: string }) {
  ...
  if (ov === undefined) return <div style={{ fontSize: 12, color: '#7f96ab', padding: '8px 2px' }}>Loading devices…</div>;
  ...
  return (
    <div style={{ background: WELL, border: '1px solid rgba(140,170,200,0.18)', borderRadius: 16, padding: 12, marginTop: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
        ...
        <span style={{ fontSize: 11, color: '#98abbd' }}>{health.online}/{health.total} online</span>
      </div>

      {devices.length === 0 ? <div style={{ fontSize: 11.5, color: '#6f8397', padding: '6px 2px' }}>No devices reported.</div> : (
          ...
          {devices.map((d, i) => (
            <div key={d.mac ?? d.name ?? i} style={{ ...TILE, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                <div style={{ color: '#98abbd', fontSize: 10 }}>{[d.model, d.version ? `v${d.version}` : null, d.uptime_s ? `up ${uptime(d.uptime_s)}` : null].filter(Boolean).join(' · ')}</div>
              </div>
              {d.clients != null && <span style={{ fontSize: 10.5, color: '#7f96ab' }}>{d.clients} cl</span>}
            </div>
          ))}
        </div>
      )}
      ...
      {showClients && (
        clients == null ? <div style={{ fontSize: 11.5, color: '#7f96ab', marginTop: 8 }}>Loading clients…</div>
          : clients.length === 0 ? <div style={{ fontSize: 11.5, color: '#6f8397', marginTop: 8 }}>No clients reported.</div>
          : <div style={{ display: 'flex', flexDirection: 'column', gap: 3, marginTop: 8, maxHeight: 240, overflowY: 'auto' }}>
              {clients.slice(0, 200).map((c, i) => (
                <div key={c.mac ?? i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5, padding: '3px 0', borderBottom: '1px solid rgba(140,170,200,0.07)' }}>
                  ...
                  {c.ip && <span style={{ color: '#7f96ab', marginLeft: 'auto' }}>{c.ip}</span>}
                </div>
              ))}
            </div>
      )}
      <div style={{ fontSize: 9.5, color: '#6f8397', marginTop: 8 }}>Live monitoring via UniFi Site Manager. Device reboot / PoE power-cycle needs a console Integration key (corporate setup) — coming next.</div>
    </div>
  );
}
