          ...
          {FILTERS.map(f => {
            ...
            return (
              <button key={f.id} type="button" onClick={() => { setFilter(f.id); setOpenId(null); setSelectedId(null) }}
                className={`rounded-lg px-3 py-1 text-[11px] font-semibold transition-all ${active ? 'border border-[#5FB8E0]/45 bg-[#2f7fb8]/30 text-[#bfe6ff]' : 'text-slate-400 hover:text-slate-200'}`}>
                {f.label}
              </button>
            )
          })}
