...
export function SiteCameraEvents({ siteId }: { siteId: string }) {
  ...
  return (
    <div style={{ background: WELL, border: '1px solid rgba(140,170,200,0.18)', borderRadius: 16, padding: 12, marginTop: 12 }}>
      ...
      {events == null ? <div style={{ fontSize: 12, color: '#7f96ab', padding: '8px 2px' }}>Loading events…</div>
        : shown.length === 0 ? <div style={{ fontSize: 11.5, color: '#6f8397', padding: '8px 2px' }}>No camera events in this window.</div>
            ...
            {shown.map((e, i) => {
              ...
              return (
                <div key={e.id ?? i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5, padding: '5px 0', borderBottom: '1px solid rgba(140,170,200,0.08)' }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: alert ? '#f2637e' : '#7ee0a8', flexShrink: 0 }} />
                  <span aria-hidden>{glyph(e.label)}</span>
                  <span style={{ color: '#e2ebf4' }}>{e.label}{e.ongoing ? ' · ongoing' : ''}</span>
                  {e.cameraName && <span style={{ color: '#98abbd' }}>· {e.cameraName}</span>}
                  {e.when && <span style={{ color: '#7f96ab', marginLeft: 'auto' }}>{fmt(e.when)}</span>}
                </div>
              );
            })}
          </div>}
