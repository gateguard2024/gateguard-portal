    ...
    <div className="border-t border-white/5">
      <button type="button" onClick={() => setOpen(o => !o)} className="flex w-full items-center justify-between px-1 py-2.5 transition-colors hover:text-white">
        <span className="flex items-center gap-2 text-[12.5px] font-medium text-slate-200">
          <ChevronRight size={13} className="transition-transform" style={{ color: '#9FD8EC', transform: open ? 'rotate(90deg)' : 'none' }} />
          {title}
        </span>
      ...
      {open && (
        <div className="space-y-0.5 pb-2 pl-6 pr-0.5">
          {loading && <div className="px-1 py-1 text-[11px] text-slate-300">Loading…</div>}
          {!loading && rows && rows.length === 0 && <div className="px-1 py-1 text-[11px] text-slate-300">Nothing here.</div>}
          {rows?.map(row => (
            <a key={row.id} href={row.link || '#'} className="block rounded-md px-2 py-1.5 transition-colors hover:bg-white/5">
              ...
              {row.meta && <span className="block truncate text-[10px] text-slate-200">{row.meta}</span>}
            </a>
          ))}
        </div>
      )}
    ...
    <div className="hidden lg:block">
      ...
      >
        <div className="px-4 pb-3">
          ...
          <div className="text-[10px] uppercase tracking-[0.16em] text-slate-200">Top 5 · then your queues</div>
        </div>

        <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-6">
          <div className="space-y-0.5">
            {!loaded && <div className="px-1 py-2 text-[12px] text-slate-300">Loading…</div>}
            {loaded && priorities.length === 0 && <div className="rounded-lg px-2 py-3 text-[12px] text-slate-200">You&apos;re all caught up.</div>}
            {priorities.map((item, i) => (
              <a key={`${item.type}-${item.id}`} href={item.link || '#'} className="flex items-start gap-2.5 rounded-lg px-2 py-2 transition-colors hover:bg-white/5">
                <span className="mt-px w-3.5 shrink-0 text-right text-[11px] font-semibold text-slate-300">{i + 1}</span>
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block truncate text-[10.5px] text-slate-200">{TYPE_LABEL[item.type] ?? item.type}{item.date ? ` · ${relDue(item.date)}` : ''}</span>
                </span>
              </a>
            ))}
          </div>
