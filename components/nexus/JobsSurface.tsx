...
const JOB_CARDS: JobCard[] = [
  ...
  { focus: 'openJobs', title: 'Work Orders', subtitle: 'Create, assign, schedule, and track every job.', hex: '#5FB8E0', glyph: 'job-alert', actionLabel: 'Open →', badgeKey: 'openJobs', badgeLabel: 'Active', opsTab: 'Work Orders' },
          ...
          >
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <div className="mt-1 text-[11px]" style={{ color: 'rgba(255,255,255,0.82)' }}>{jobDisplaySubtitle(job)}</div>
                {(job.due_date || job.scheduled_date || job.assignee_name) && (
                  <div className="mt-2 text-[10px]" style={{ color: 'rgba(255,255,255,0.82)' }}>
                    {[
            ...
            {job.notes && <div className="mt-3 line-clamp-2 text-[11px] leading-relaxed" style={{ color: 'rgba(255,255,255,0.82)' }}>{job.notes}</div>}
          </div>
        )
      })}
    </div>
  ...
  return (
    <button
      ...
      style={muted || disabled
        ...
        : { background: 'linear-gradient(135deg, rgba(95,184,224,0.20), rgba(95,184,224,0.10))', border: '1px solid rgba(95,184,224,0.24)', color: '#86efac', boxShadow: '0 0 18px rgba(95,184,224,0.12)' }}
    >
      {label}
    </button>
  )
          ...
          actions={
            <>
              {selectedBoardJob ? (
                ...
                <div className="rounded-2xl p-3 text-[11px]" style={{ background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.82)' }}>Select a job first.</div>
              )}
