    ...
    <div className="rounded-3xl p-4" style={{ background: 'repeating-linear-gradient(90deg,rgba(255,255,255,0.04) 0 1px,transparent 1px 4px), linear-gradient(180deg,#2b3c52,#1e2a3a)', border: '1px solid rgba(140,170,200,0.22)', boxShadow: '0 14px 30px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.12)' }}>
      <div className="mb-3 flex items-center justify-between">
        ...
        {typeof count === 'number' && count > 0 && (
          <div className="rounded-full px-2.5 py-1 text-[10px] font-semibold" style={{ background: 'rgba(95,184,224,0.16)', color: '#9FD8EC' }}>
            {count}
          </div>
        )}
      </div>
...
function MiniRow({ title, subtitle, meta }: { title: string; subtitle?: string; meta?: string }) {
  return (
    <div className="rounded-2xl p-3" style={{ background: 'linear-gradient(180deg,#22303f,#1a2532)', border: '1px solid rgba(140,170,200,0.2)', boxShadow: 'inset 0 1px 3px rgba(0,0,0,0.4)' }}>
...
function ActionButton({ title, subtitle, active, onClick, tone = 'default' }: { title: string; subtitle: string; active: boolean; onClick: () => void; tone?: 'default' | 'danger' }) {
  ...
  return (
    <button
      ...
      style={{
        background: active
          ? isDanger ? 'rgba(239,68,68,0.15)' : 'rgba(95,184,224,0.18)'
          : isDanger ? 'rgba(239,68,68,0.10)' : 'rgba(40,55,74,0.55)',
        border: active
          ? isDanger ? '1px solid rgba(239,68,68,0.38)' : '1px solid rgba(95,184,224,0.36)'
          : isDanger ? '1px solid rgba(239,68,68,0.24)' : '1px solid rgba(140,170,200,0.22)',
  ...
  return (
    <Section title="Possible Duplicates" count={totalMatches}>
      ...
      <div className="space-y-4">
        {contacts.length > 0 && <div><div className="mb-2 text-[10px] uppercase tracking-[0.16em]" style={{ color: '#9FD8EC' }}>Contacts</div><ListBlock records={contacts} emptyText="No contact matches." render={contact => <MiniRow title={personName(contact)} subtitle={[contact.title, contact.email, contact.phone].filter(Boolean).join(' • ')} meta="Contact" />} /></div>}
        {company && <div><div className="mb-2 text-[10px] uppercase tracking-[0.16em]" style={{ color: '#9FD8EC' }}>Company</div><MiniRow title={val(company.name, 'Possible company')} subtitle={[company.website, company.city, company.state].filter(Boolean).join(' • ')} meta="Company" /></div>}
        {possibleProperties.length > 0 && <div><div className="mb-2 text-[10px] uppercase tracking-[0.16em]" style={{ color: '#9FD8EC' }}>Properties</div><ListBlock records={possibleProperties} emptyText="No property matches." render={property => <MiniRow title={val(property.name, 'Possible property')} subtitle={[property.address, property.city, property.state].filter(Boolean).join(', ')} meta={property.unit_count ? `${property.unit_count} units` : 'Property'} />} /></div>}
        {sites.length > 0 && <div><div className="mb-2 text-[10px] uppercase tracking-[0.16em]" style={{ color: '#9FD8EC' }}>Sites</div><ListBlock records={sites} emptyText="No site matches." render={site => <MiniRow title={val(site.name, 'Possible site')} subtitle={[site.address, site.city, site.state].filter(Boolean).join(', ')} meta={site.units ? `${site.units} units` : 'Site'} />} /></div>}
        {opportunities.length > 0 && <div><div className="mb-2 text-[10px] uppercase tracking-[0.16em]" style={{ color: '#9FD8EC' }}>Opportunities</div><ListBlock records={opportunities} emptyText="No opportunity matches." render={opp => <MiniRow title={val(opp.name, 'Possible opportunity')} subtitle={[opp.account_name, opp.next_step].filter(Boolean).join(' • ')} meta={val(opp.stage, 'Opportunity')} />} /></div>}
      </div>
    </Section>
  )
}
...
export function LeadGlassWindow({ data, onBack, onRefresh, onOpenOpportunity }: { data: LeadGlassData; onBack: () => void; onRefresh?: () => Promise<void> | void; onOpenOpportunity?: (id: string) => void | Promise<void> }) {
  ...
  async function openReassign() {
    setActiveAction(activeAction === 'reassign_lead' ? null : 'reassign_lead')
    if (assignUsers.length === 0) {
      setAssignLoading(true)
      try { const r = await fetch('/api/admin/users'); const d = await r.json().catch(() => ({})); setAssignUsers(Array.isArray(d?.users) ? d.users : []) } finally { setAssignLoading(false) }
    }
  }
  ...
  const [detailLeadType, setDetailLeadType] = useState(String(lead.lead_type ?? ''))
  const [detailEntryPoints, setDetailEntryPoints] = useState(lead.entry_points != null ? String(lead.entry_points) : '')
  const [detailCameras, setDetailCameras] = useState(lead.cameras != null ? String(lead.cameras) : '')
  const [detailMrr, setDetailMrr] = useState(lead.mrr != null ? String(lead.mrr) : '')
  ...
  function chooseAction(action: LeadAction) {
    ...
    if (action === 'delete_lead') setDeleteConfirmText('')
    setActiveAction(activeAction === action ? null : action)
    setActionMessage(null)
  }

  // Name we show the user in the delete panel — never a raw id.
  const leadDisplayName =
    String(lead.contact_name ?? lead.name ?? lead.company_name ?? lead.property_name ?? '').trim() || 'this lead'
  ...
  async function deleteLead() {
    ...
    try {
      const res = await fetch('/api/trash', {
        ...
        body: JSON.stringify({ table: 'leads', ids: [lead.id], action: 'delete' }),
      })
      const result = await res.json().catch(() => ({}))
      if (!res.ok || result?.success === false) {
        throw new Error(result?.error ?? result?.message ?? 'We could not delete this lead. Nothing was changed — please try again.')
      }
      ...
      setActionMessage(err instanceof Error ? err.message : 'We could not delete this lead. Nothing was changed — please try again.')
    } finally {
      setActionBusy(false)
    }
  }
  ...
  async function submitLeadDetails() {
    ...
    setActionBusy(true); setActionMessage(null)
    try {
      // MRR = Units × $10 + Cams × $100 (a typed MRR wins); EMC = MRR × 10%
      const uu = parseInt(detailUnitCount, 10); const cc = parseInt(detailCameras, 10)
      const autoMrr = (!isNaN(uu) || !isNaN(cc)) ? (isNaN(uu) ? 0 : uu) * 10 + (isNaN(cc) ? 0 : cc) * 100 : null
      const mrrManual = detailMrr.trim() !== '' ? parseFloat(detailMrr) : null
      const mrrVal = (mrrManual != null && !isNaN(mrrManual)) ? mrrManual : autoMrr
      const emcVal = mrrVal != null ? Math.round(mrrVal * 0.10 * 100) / 100 : null
      const res = await fetch(`/api/nexus/opps/lead-window/${leadId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'update_details', contact_name: detailContactName, company_name: detailCompanyName, email: detailEmail, phone: detailPhone, location: detailLocation, property_type: detailPropertyType, unit_count: detailUnitCount, notes: detailNotes, source: detailContactSource, lead_type: detailLeadType, entry_points: detailEntryPoints, cameras: detailCameras, mrr: mrrVal != null ? String(mrrVal) : '', pcr: emcVal != null ? String(emcVal) : '' }) })
      const result = await res.json().catch(() => ({}))
  ...
  return (
    ...
    <div className="space-y-4 pb-28 rounded-[2rem] p-4 sm:p-5" style={WIN_FRAME}>
      {activeAction === 'edit_details' && (
        <div className="fixed inset-0 z-[120] overflow-y-auto px-4 py-4 sm:py-8" style={{ background: 'rgba(0,0,0,0.72)', backdropFilter: 'blur(10px)', WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain' }}>
          <div className="mx-auto flex min-h-full w-full max-w-2xl items-start justify-center">
            <div className="w-full overflow-hidden rounded-[2rem]" style={{ background: 'linear-gradient(180deg, rgba(18,28,52,0.98), rgba(8,14,28,0.98))', border: '1px solid rgba(95,184,224,0.32)', boxShadow: '0 30px 100px rgba(0,0,0,0.55)' }}>
              <div className="sticky top-0 z-10 flex items-start justify-between gap-3 p-5" style={{ background: 'linear-gradient(180deg, rgba(18,28,52,0.98), rgba(18,28,52,0.92))', borderBottom: '1px solid rgba(95,184,224,0.18)' }}><div><div className="text-[10px] uppercase tracking-[0.22em]" style={{ color: 'rgba(159,216,236,0.9)' }}>Edit Lead Details</div><h3 className="mt-1 text-lg font-semibold" style={{ color: 'rgba(255,255,255,0.96)' }}>Fill in the basics</h3><p className="mt-1 text-xs" style={{ color: 'rgba(255,255,255,0.62)' }}>Add the contact, phone, email, property basics, and notes.</p></div><button type="button" onClick={() => setActiveAction(null)} className="rounded-full px-3 py-1.5 text-[11px]" style={{ background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.78)' }}>Close</button></div>
              <div className="max-h-[calc(100vh-11rem)] overflow-y-auto p-5" style={{ WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain' }}><div className="grid gap-2 sm:grid-cols-2"><input value={detailContactName} onChange={e => setDetailContactName(e.target.value)} placeholder="Contact name" className="rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><input value={detailPhone} onChange={e => setDetailPhone(e.target.value)} placeholder="Phone" className="rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><input value={detailEmail} onChange={e => setDetailEmail(e.target.value)} placeholder="Email" className="rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><input value={detailCompanyName} onChange={e => setDetailCompanyName(e.target.value)} placeholder="Property / company" className="rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><input value={detailLocation} onChange={e => setDetailLocation(e.target.value)} placeholder="Address / location" className="rounded-xl px-3 py-2 text-sm outline-none sm:col-span-2" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><input value={detailUnitCount} onChange={e => setDetailUnitCount(e.target.value)} placeholder="Units" className="rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><select value={detailLeadType} onChange={e => setDetailLeadType(e.target.value)} className="rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }}><option value="">Type…</option>{['MDU','SFH','Commercial','Mixed-Use','Gated','HOA'].map(t => <option key={t} value={t}>{t}</option>)}</select><input value={detailEntryPoints} onChange={e => setDetailEntryPoints(e.target.value)} placeholder="Entry points" className="rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><input value={detailCameras} onChange={e => setDetailCameras(e.target.value)} placeholder="Cameras" className="rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><input value={detailMrr} onChange={e => setDetailMrr(e.target.value)} placeholder="MRR ($/mo)" className="rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><input value={detailPropertyType} onChange={e => setDetailPropertyType(e.target.value)} placeholder="Property type" className="rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><textarea value={detailNotes} onChange={e => setDetailNotes(e.target.value)} placeholder="Notes / need — include address discrepancies or ARIA vs Google differences here" rows={4} className="rounded-xl px-3 py-2 text-sm outline-none resize-none sm:col-span-2" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><div className="sm:col-span-2"><label className="block mb-1 text-[10px] uppercase tracking-[0.14em]" style={{ color: 'rgba(255,255,255,0.58)' }}>Contact Source</label><select value={detailContactSource} onChange={e => setDetailContactSource(e.target.value)} className="w-full rounded-xl px-3 py-2 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.24)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }}><option value="">Unknown</option><option value="nexus_aria">ARIA</option><option value="nexus_google">Google</option><option value="nexus_outbound">Outbound / Manual</option><option value="nexus_phone">Phone Call</option><option value="nexus_walk_in">Walk-In</option></select></div></div></div>
              <div className="sticky bottom-0 flex justify-end gap-2 p-4" style={{ background: 'linear-gradient(0deg, rgba(8,14,28,0.98), rgba(8,14,28,0.90))', borderTop: '1px solid rgba(95,184,224,0.18)' }}><button type="button" onClick={() => setActiveAction(null)} className="rounded-full px-4 py-2 text-xs" style={{ background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.72)' }}>Cancel</button><button type="button" disabled={actionBusy} onClick={submitLeadDetails} className="rounded-full px-4 py-2 text-xs disabled:opacity-40" style={{ background: '#2f7fb8', color: 'white' }}>{actionBusy ? 'Saving...' : 'Save Changes'}</button></div>
            </div>
          </div>
        </div>
      )}

      <div className="rounded-[2rem] p-5" style={{ background: 'repeating-linear-gradient(90deg,rgba(255,255,255,0.04) 0 1px,transparent 1px 4px), linear-gradient(145deg,#33465e,#1e2a3a)', border: '1px solid rgba(95,184,224,0.32)', boxShadow: '0 20px 70px rgba(0,0,0,0.24), inset 0 1px 0 rgba(255,255,255,0.08)' }}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-[10px] uppercase tracking-[0.24em]" style={{ color: 'rgba(159,216,236,0.9)' }}>Lead</div><h3 className="mt-2 text-3xl font-semibold leading-tight" style={{ color: 'rgba(255,255,255,0.98)' }}>{val(lead.contact_name ?? lead.name, 'Unnamed lead')}</h3><div className="mt-2 text-base" style={{ color: 'rgba(255,255,255,0.74)' }}>{val(lead.company_name ?? company?.name, 'No company attached')}</div>{lead.location && <div className="mt-1 text-sm" style={{ color: 'rgba(255,255,255,0.66)' }}>{String(lead.location)}</div>}<div className="mt-2 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px]" style={{ background: 'rgba(95,184,224,0.12)', border: '1px solid rgba(95,184,224,0.28)', color: '#9FD8EC' }}>Rep: {val(lead.assigned_to_name, 'Unassigned')}{canReassign && <span style={{ color: 'rgba(255,255,255,0.4)' }}> · reassignable</span>}</div></div><div className="rounded-2xl px-4 py-3 text-right" style={{ background: 'linear-gradient(180deg,#22303f,#1a2532)', border: '1px solid rgba(255,255,255,0.10)' }}><div className="text-[10px] uppercase tracking-[0.18em]" style={{ color: 'rgba(255,255,255,0.58)' }}>Status</div><div className="mt-1 text-sm font-semibold capitalize" style={{ color: 'rgba(255,255,255,0.92)' }}>{statusLabel(lead.stage)}</div></div></div>
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4"><MiniStat label="Source" value={leadSourceLabel(lead.source)} onEdit={() => chooseAction('edit_details')} /><MiniStat label="Units" value={val(lead.unit_count, 'Unknown')} onEdit={() => chooseAction('edit_details')} /><MiniStat label="Phone" value={val(lead.phone)} onEdit={() => chooseAction('edit_details')} /><MiniStat label="Email" value={val(lead.email)} onEdit={() => chooseAction('edit_details')} /></div><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-5"><MiniStat label="Type" value={val(lead.lead_type)} onEdit={() => chooseAction('edit_details')} /><MiniStat label="Entry Pts" value={val(lead.entry_points)} onEdit={() => chooseAction('edit_details')} /><MiniStat label="Cams" value={val(lead.cameras)} onEdit={() => chooseAction('edit_details')} /><MiniStat label="MRR" value={(() => { const m = lead.mrr != null ? Number(lead.mrr) : ((lead.unit_count != null || lead.cameras != null) ? Number(lead.unit_count || 0) * 10 + Number(lead.cameras || 0) * 100 : null); return m != null ? `$${Math.round(m).toLocaleString()}` : '—' })()} onEdit={() => chooseAction('edit_details')} /><MiniStat label="EMC" value={(() => { const m = lead.mrr != null ? Number(lead.mrr) : ((lead.unit_count != null || lead.cameras != null) ? Number(lead.unit_count || 0) * 10 + Number(lead.cameras || 0) * 100 : null); return m != null ? `$${Math.round(m * 0.10).toLocaleString()}` : '—' })()} /></div><div className="mt-3 flex items-stretch gap-1.5 rounded-2xl px-3 py-2.5" style={{ background: 'linear-gradient(180deg,#22303f,#1a2532)', border: '1px solid rgba(140,170,200,0.2)' }}>{[{ label: 'New', at: lead.created_at as string | null, done: true }, { label: 'Contacted', at: lead.contacted_at as string | null, done: !!lead.contacted_at }, { label: 'Info Sent', at: lead.sent_info_at as string | null, done: !!lead.sent_info_at }, { label: 'Visited', at: lead.visited_at as string | null, done: !!lead.visited_at }, { label: 'Converted', at: (lead.converted_at as string | null), done: !!(lead.converted_at || lead.opportunity_id) }].map((st, i, arr) => (<div key={st.label} className="flex flex-1 items-center gap-1.5"><div role="button" tabIndex={0} title={`Move to ${st.label}`} onClick={() => { const m: Record<string, Record<string, unknown>> = { New: { action: 'update_status', stage: 'new' }, Contacted: { action: 'update_status', stage: 'contacted' }, 'Info Sent': { action: 'send_info' }, Visited: { action: 'log_visit' }, Converted: { action: 'create_opportunity' } }; const a = m[st.label]; if (a) void submitLeadAction(a); }} className="flex flex-col items-center text-center cursor-pointer transition-opacity hover:opacity-80" style={{ minWidth: 52 }}><div className="flex h-6 w-6 items-center justify-center rounded-full text-[11px] font-bold" style={{ background: st.done ? '#2f8f5f' : 'rgba(255,255,255,0.06)', border: st.done ? '1px solid rgba(126,224,168,0.5)' : '1px solid rgba(255,255,255,0.14)', color: st.done ? '#eafff3' : 'rgba(255,255,255,0.4)' }}>{st.done ? '\u2713' : i + 1}</div><div className="mt-1 text-[9px] uppercase tracking-wide" style={{ color: st.done ? '#9FD8EC' : 'rgba(255,255,255,0.4)' }}>{st.label}</div><div className="text-[8px]" style={{ color: 'rgba(255,255,255,0.4)' }}>{st.at ? new Date(st.at).toLocaleDateString([], { month: 'short', day: 'numeric' }) : ''}</div></div>{i < arr.length - 1 && <div className="h-px flex-1" style={{ background: st.done ? 'rgba(126,224,168,0.4)' : 'rgba(140,170,200,0.18)' }} />}</div>))}</div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="space-y-4">
          ...
          <Section title="Attachments" count={attachments.length + surveys.length}>
            <label className="mb-2 inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-semibold" style={{ background: 'rgba(95,184,224,0.12)', border: '1px solid rgba(95,184,224,0.28)', color: '#9FD8EC' }}>
        ...
        <div className="space-y-4"><Section title="Actions"><div className="space-y-2"><ActionButton title="Edit Lead Details" subtitle="Add phone, email, address, and notes." active={activeAction === 'edit_details'} onClick={() => chooseAction('edit_details')} /><ActionButton title="Create Opportunity" subtitle="This looks like a real deal." active={activeAction === 'create_opportunity'} onClick={() => chooseAction('create_opportunity')} /><ActionButton title="Add Note" subtitle="Remember something about this lead." active={activeAction === 'add_note'} onClick={() => chooseAction('add_note')} /><ActionButton title="Log Call" subtitle="Capture what happened on a call." active={activeAction === 'log_call'} onClick={() => chooseAction('log_call')} /><ActionButton title="Log Site Visit" subtitle="Record a completed site visit." active={false} onClick={() => submitLeadAction({ action: 'log_visit' })} /><ActionButton title="Send Info" subtitle="Sent them info — moves to Sent Info." active={false} onClick={() => submitLeadAction({ action: 'send_info' })} /><ActionButton title="Create Task / Follow-Up" subtitle="Put the next touch on your list." active={activeAction === 'schedule_followup'} onClick={() => chooseAction('schedule_followup')} /><ActionButton title="Change Lead Status" subtitle="Move this lead forward." active={activeAction === 'update_status'} onClick={() => chooseAction('update_status')} />{canReassign && <ActionButton title="Assign / Reassign Lead" subtitle="Send this lead to another rep." active={activeAction === 'reassign_lead'} onClick={() => void openReassign()} />}<ActionButton title="Contact Not Found" subtitle="Needs contact research." active={false} tone="danger" onClick={() => submitLeadAction({ action: 'add_note', note: 'Contact not found — needs research.' })} /><ActionButton title="Delete Lead" subtitle="Move to Deleted Items — you can restore it." active={activeAction === 'delete_lead'} tone="danger" onClick={() => chooseAction('delete_lead')} />
          {(activeAction === 'delete_lead' || activeAction === 'create_opportunity' || activeAction === 'add_note' || activeAction === 'log_call' || activeAction === 'schedule_followup' || activeAction === 'update_status' || activeAction === 'reassign_lead') && (
          <div className="fixed inset-0 z-[120] flex items-start justify-center overflow-y-auto px-4 py-10" style={{ background: 'rgba(0,0,0,0.72)', backdropFilter: 'blur(10px)' }} onClick={() => setActiveAction(null)}>
          <div className="w-full max-w-lg space-y-3" style={{ marginTop: '7vh' }} onClick={e => e.stopPropagation()}>
          {activeAction === 'delete_lead' && (
            <div className="rounded-2xl p-3 space-y-3" style={{ background: 'rgba(11,23,40,0.92)', border: '1px solid rgba(244,63,94,0.34)' }}>
              <div className="text-[13px] font-semibold" style={{ color: 'rgba(255,255,255,0.94)' }}>Delete this lead?</div>
              <p className="text-[11px] leading-relaxed" style={{ color: 'rgba(255,255,255,0.72)' }}>
                This removes <span style={{ color: 'rgba(255,255,255,0.94)', fontWeight: 600 }}>{leadDisplayName}</span> from your lists. It moves to Deleted Items where an admin can restore it.
              </p>
              <div>
                <label htmlFor="lead-delete-confirm" className="block text-[10px] uppercase tracking-[0.14em] mb-1" style={{ color: 'rgba(255,255,255,0.60)' }}>
                ...
                <input
                  ...
                  style={{ background: 'rgba(19,27,46,0.9)', border: '1px solid rgba(244,63,94,0.28)', color: 'rgba(255,255,255,0.9)' }}
                />
              </div>
              <div className="flex gap-2">
                <button type="button" onClick={() => { setActiveAction(null); setDeleteConfirmText('') }} className="rounded-full px-3 py-1.5 text-[11px]" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.7)' }}>Cancel</button>
                <button
                  type="button"
                  disabled={actionBusy || deleteConfirmText !== 'DELETE'}
                  onClick={() => void deleteLead()}
                  className="rounded-full px-3 py-1.5 text-[11px] font-semibold disabled:cursor-not-allowed disabled:opacity-40"
                  style={{ background: '#E11D48', color: 'white' }}
                >
                  {actionBusy ? 'Deleting...' : 'Delete lead'}
                </button>
              </div>
            </div>
          )}
          {activeAction === 'create_opportunity' && <div className="rounded-2xl p-3 space-y-2" style={{ background: 'rgba(0,0,0,0.20)', border: '1px solid rgba(95,184,224,0.24)' }}><p className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.76)' }}>Nexus will create an opportunity and carry forward this lead data.</p><div className="flex gap-2"><button type="button" onClick={() => setActiveAction(null)} className="rounded-full px-3 py-1.5 text-[11px]" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.7)' }}>Cancel</button><button type="button" disabled={actionBusy} onClick={() => submitLeadAction({ action: 'create_opportunity' })} className="rounded-full px-3 py-1.5 text-[11px] disabled:opacity-40" style={{ background: '#2f7fb8', color: 'white' }}>{actionBusy ? 'Creating...' : 'Create Opportunity'}</button></div></div>}
          {activeAction === 'add_note' && <div className="rounded-2xl p-3 space-y-2" style={{ background: 'rgba(0,0,0,0.20)', border: '1px solid rgba(255,255,255,0.10)' }}><textarea value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="What should Nexus remember?" rows={3} className="w-full rounded-xl px-3 py-2 text-xs outline-none resize-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><div className="flex gap-2"><button type="button" onClick={() => setActiveAction(null)} className="rounded-full px-3 py-1.5 text-[11px]" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.7)' }}>Cancel</button><button type="button" disabled={actionBusy || !noteText.trim()} onClick={() => submitLeadAction({ action: 'add_note', note: noteText })} className="rounded-full px-3 py-1.5 text-[11px] disabled:opacity-40" style={{ background: '#2f7fb8', color: 'white' }}>{actionBusy ? 'Saving...' : 'Save Note'}</button></div></div>}
          {activeAction === 'log_call' && <div className="rounded-2xl p-3 space-y-2" style={{ background: 'rgba(0,0,0,0.20)', border: '1px solid rgba(95,184,224,0.24)' }}><label className="block text-[10px] uppercase tracking-[0.14em] mb-1" style={{ color: 'rgba(255,255,255,0.60)' }}>Call summary</label><textarea value={callSummary} onChange={e => setCallSummary(e.target.value)} placeholder="What happened on the call?" rows={3} className="w-full rounded-xl px-3 py-2 text-xs outline-none resize-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><div className="flex gap-2"><input value={callOutcome} onChange={e => setCallOutcome(e.target.value)} placeholder="Outcome (optional)" className="flex-1 rounded-xl px-3 py-2 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><input value={callDuration} onChange={e => setCallDuration(e.target.value)} placeholder="Mins" className="w-20 rounded-xl px-3 py-2 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /></div><div className="flex gap-2"><button type="button" onClick={() => setActiveAction(null)} className="rounded-full px-3 py-1.5 text-[11px]" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.7)' }}>Cancel</button><button type="button" disabled={actionBusy || !callSummary.trim()} onClick={() => submitLeadAction({ action: 'log_call', summary: callSummary, outcome: callOutcome, duration: callDuration })} className="rounded-full px-3 py-1.5 text-[11px] disabled:opacity-40" style={{ background: '#2f7fb8', color: 'white' }}>{actionBusy ? 'Saving...' : 'Log Call'}</button></div></div>}
          {activeAction === 'schedule_followup' && <div className="rounded-2xl p-3 space-y-2" style={{ background: 'rgba(0,0,0,0.20)', border: '1px solid rgba(95,184,224,0.24)' }}><label className="block text-[10px] uppercase tracking-[0.14em] mb-1" style={{ color: 'rgba(255,255,255,0.60)' }}>Follow-up task</label><input value={followupTitle} onChange={e => setFollowupTitle(e.target.value)} placeholder="What's the next touch?" className="w-full rounded-xl px-3 py-2 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><input type="date" value={followupDate} onChange={e => setFollowupDate(e.target.value)} className="w-full rounded-xl px-3 py-2 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><textarea value={followupNotes} onChange={e => setFollowupNotes(e.target.value)} placeholder="Notes (optional)" rows={2} className="w-full rounded-xl px-3 py-2 text-xs outline-none resize-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }} /><div className="flex gap-2"><button type="button" onClick={() => setActiveAction(null)} className="rounded-full px-3 py-1.5 text-[11px]" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.7)' }}>Cancel</button><button type="button" disabled={actionBusy || !followupTitle.trim()} onClick={() => submitLeadAction({ action: 'schedule_followup', title: followupTitle, due_date: followupDate, body: followupNotes })} className="rounded-full px-3 py-1.5 text-[11px] disabled:opacity-40" style={{ background: '#2f7fb8', color: 'white' }}>{actionBusy ? 'Saving...' : 'Schedule'}</button></div></div>}
          {activeAction === 'update_status' && <div className="rounded-2xl p-3 space-y-2" style={{ background: 'rgba(0,0,0,0.20)', border: '1px solid rgba(255,255,255,0.10)' }}><label className="block text-[10px] uppercase tracking-[0.14em] mb-1" style={{ color: 'rgba(255,255,255,0.60)' }}>Change lead status to</label><select value={statusStage} onChange={e => setStatusStage(e.target.value)} className="w-full rounded-xl px-3 py-2 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.9)' }}>{LEAD_STATUS_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select><div className="flex gap-2"><button type="button" onClick={() => setActiveAction(null)} className="rounded-full px-3 py-1.5 text-[11px]" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.7)' }}>Cancel</button><button type="button" disabled={actionBusy} onClick={() => submitLeadAction({ action: 'update_status', stage: statusStage })} className="rounded-full px-3 py-1.5 text-[11px] disabled:opacity-40" style={{ background: '#2f7fb8', color: 'white' }}>{actionBusy ? 'Saving...' : 'Save Status'}</button></div></div>}
          {activeAction === 'reassign_lead' && <div className="rounded-2xl p-3 space-y-2" style={{ background: 'rgba(0,0,0,0.20)', border: '1px solid rgba(255,255,255,0.10)' }}><label className="block text-[10px] uppercase tracking-[0.14em] mb-1" style={{ color: 'rgba(255,255,255,0.60)' }}>Assign this lead to</label>{assignLoading ? <div className="py-3 text-center text-[11px]" style={{ color: 'rgba(255,255,255,0.5)' }}>Loading team…</div> : assignUsers.length === 0 ? <div className="py-3 text-center text-[11px]" style={{ color: 'rgba(255,255,255,0.5)' }}>No teammates available.</div> : <div className="max-h-56 overflow-y-auto rounded-xl" style={{ border: '1px solid rgba(255,255,255,0.10)' }}>{assignUsers.map(u => <button key={u.id} type="button" disabled={actionBusy} onClick={() => submitLeadAction({ action: 'reassign_lead', assignee_id: u.id, assignee_name: u.full_name || u.email })} className="flex w-full items-center justify-between border-b px-3 py-2 text-left transition-colors hover:bg-white/5 disabled:opacity-40" style={{ borderColor: 'rgba(255,255,255,0.06)' }}><span className="text-[12px]" style={{ color: 'rgba(255,255,255,0.9)' }}>{u.full_name || u.email}</span><span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.82)' }}>{u.email}</span></button>)}</div>}<div className="flex justify-end"><button type="button" onClick={() => setActiveAction(null)} className="rounded-full px-3 py-1.5 text-[11px]" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.7)' }}>Close</button></div></div>}
          </div></div>
          )}
        </div></Section>{actionMessage && <div className="rounded-2xl p-3 text-xs" style={{ background: 'rgba(95,184,224,0.12)', border: '1px solid rgba(95,184,224,0.22)', color: 'rgba(255,255,255,0.82)' }}>{actionMessage}</div>}</div>
      </div>
    </div>
    </>
  )
