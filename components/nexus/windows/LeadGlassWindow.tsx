...
function ActionButton({ title, subtitle, active, onClick }: { title: string; subtitle: string; active: boolean; onClick: () => void }) {
  return (
    <button
      ...
      style={{
        background: active ? 'rgba(107,126,255,0.14)' : 'rgba(52,211,153,0.08)',
        border: active ? '1px solid rgba(107,126,255,0.32)' : '1px solid rgba(52,211,153,0.18)',
  ...
  return (
    <Section title="Possible Matches — Duplicate Guard" count={totalMatches}>
      <p className="mb-4 text-[11px] leading-relaxed" style={{ color: 'rgba(255,255,255,0.38)' }}>
        Before creating new records, check these existing matches. Update and link instead of duplicating. Display only — no actions yet.
      </p>
      <div className="space-y-4">
        <div>
          <div className="mb-2 text-[10px] uppercase tracking-[0.16em]" style={{ color: 'rgba(107,126,255,0.7)' }}>Possible Contact Matches ({contacts.length})</div>
        ...
        <div>
          <div className="mb-2 text-[10px] uppercase tracking-[0.16em]" style={{ color: 'rgba(107,126,255,0.7)' }}>Possible Company Matches ({company ? 1 : 0})</div>
        ...
        <div>
          <div className="mb-2 text-[10px] uppercase tracking-[0.16em]" style={{ color: 'rgba(107,126,255,0.7)' }}>Possible Property Matches ({possibleProperties.length})</div>
        ...
        <div>
          <div className="mb-2 text-[10px] uppercase tracking-[0.16em]" style={{ color: 'rgba(107,126,255,0.7)' }}>Possible Site Matches ({sites.length})</div>
        ...
        <div>
          <div className="mb-2 text-[10px] uppercase tracking-[0.16em]" style={{ color: 'rgba(107,126,255,0.7)' }}>Possible Opportunity Matches ({opportunities.length})</div>
...
export function LeadGlassWindow({
  ...
  async function submitLeadDetails() {
    ...
    setActionMessage(null)
    try {
      const res = await fetch(`/api/nexus/opps/lead-window/${leadId}/details`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          contact_name: detailContactName,
          company_name: detailCompanyName,
          email: detailEmail,
          phone: detailPhone,
          location: detailLocation,
          property_type: detailPropertyType,
          unit_count: detailUnitCount,
          notes: detailNotes,
        }),
      })
      const result = await res.json().catch(() => ({}))
    ...
    <div className="space-y-4">
      <div className="rounded-[2rem] p-5" style={{ background: 'linear-gradient(145deg, rgba(107,126,255,0.16), rgba(255,255,255,0.035))', border: '1px solid rgba(107,126,255,0.24)', boxShadow: '0 20px 70px rgba(0,0,0,0.28), inset 0 1px 0 rgba(255,255,255,0.06)' }}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <button type="button" onClick={onBack} className="mb-4 rounded-full px-3 py-1.5 text-[11px] transition-opacity hover:opacity-80" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)', color: 'rgba(255,255,255,0.58)' }}>← Back to workbench</button>
            <div className="text-[10px] uppercase tracking-[0.24em]" style={{ color: 'rgba(165,180,255,0.85)' }}>Lead</div>
            <h3 className="mt-2 text-2xl font-semibold leading-tight" style={{ color: 'rgba(255,255,255,0.94)' }}>{val(lead.contact_name ?? lead.name, 'Unnamed lead')}</h3>
            <div className="mt-2 text-sm" style={{ color: 'rgba(255,255,255,0.55)' }}>{val(lead.company_name ?? company?.name, 'No company attached')}</div>
          </div>
