    ...
    <div className="rounded-3xl p-4" style={{ background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.08)' }}>
      <div className="mb-3 flex items-center justify-between">
        ...
        {typeof count === 'number' && count > 0 && (
          <div className="rounded-full px-2 py-1 text-[10px]" style={{ background: 'rgba(95,184,224,0.12)', color: 'rgba(110,231,183,0.9)' }}>{count}</div>
        )}
      </div>
      {children}
    </div>
...
function statusColor(status: string): string {
  ...
  if (['completed', 'done'].includes(s)) return '#5FB8E0'
  if (['cancelled', 'closed'].includes(s)) return '#94a3b8'
  if (['in_progress', 'in-progress'].includes(s)) return '#5FB8E0'
...
function ActionButton({ title, subtitle, active, disabled, onClick }: { title: string; subtitle: string; active?: boolean; disabled?: boolean; onClick?: () => void }) {
  return (
    <button
      ...
      style={{
        background: active ? 'rgba(95,184,224,0.14)' : 'rgba(95,184,224,0.07)',
        border: active ? '1px solid rgba(95,184,224,0.32)' : '1px solid rgba(95,184,224,0.16)',
...
export function JobGlassWindow({
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-[2rem] p-5" style={{ background: 'linear-gradient(145deg, rgba(95,184,224,0.12), rgba(255,255,255,0.035))', border: '1px solid rgba(95,184,224,0.2)', boxShadow: '0 20px 70px rgba(0,0,0,0.28), inset 0 1px 0 rgba(255,255,255,0.06)' }}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <div className="text-[10px] uppercase tracking-[0.24em]" style={{ color: 'rgba(95,184,224,0.8)' }}>Job</div>
        ...
        <div className="space-y-4">
          <Section title="Next Best Actions">
            <div className="space-y-2">
              ...
              <label className="block w-full cursor-pointer rounded-2xl p-3 text-left transition-all hover:-translate-y-0.5" style={{ background: 'rgba(95,184,224,0.07)', border: '1px solid rgba(95,184,224,0.16)', color: 'rgba(255,255,255,0.86)', opacity: actionBusy ? 0.6 : 1 }}>
              ...
              {activeAction === 'add_note' && (
                <div className="space-y-2 rounded-2xl p-3" style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(95,184,224,0.22)' }}>
                  <textarea value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="What should Nexus remember about this job?" rows={3} className="w-full resize-none rounded-xl px-3 py-2 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.2)', color: 'rgba(255,255,255,0.88)' }} />
                  <div className="flex gap-2">
                    ...
                    <button type="button" disabled={actionBusy || !noteText.trim()} onClick={() => submitJobAction({ action: 'add_note', note: noteText })} className="rounded-full px-3 py-1.5 text-[11px] disabled:opacity-40" style={{ background: '#2f7fb8', color: 'white' }}>{actionBusy ? 'Saving...' : 'Save Note'}</button>
                  </div>
                </div>
              )}
              ...
              {activeAction === 'create_task' && (
                <div className="space-y-2 rounded-2xl p-3" style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(95,184,224,0.22)' }}>
                  <input value={taskTitle} onChange={e => setTaskTitle(e.target.value)} placeholder="What needs to get done?" className="w-full rounded-xl px-3 py-2 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.2)', color: 'rgba(255,255,255,0.88)' }} />
              ...
              {activeAction === 'schedule_visit' && (
                <div className="space-y-2 rounded-2xl p-3" style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(95,184,224,0.22)' }}>
                  ...
                  <div className="flex gap-2">
                    <input type="date" value={visitDate} onChange={e => setVisitDate(e.target.value)} className="flex-1 rounded-xl px-3 py-2 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.2)', color: 'rgba(255,255,255,0.88)' }} />
                    <input type="time" value={visitTime} onChange={e => setVisitTime(e.target.value)} className="w-28 rounded-xl px-3 py-2 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.22)', border: '1px solid rgba(95,184,224,0.2)', color: 'rgba(255,255,255,0.88)' }} />
                  </div>
              ...
              {activeAction === 'mark_complete' && (
                <div className="space-y-2 rounded-2xl p-3" style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(95,184,224,0.22)' }}>
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setActiveAction(null)} className="rounded-full px-3 py-1.5 text-[11px]" style={{ background: 'rgba(255,255,255,0.045)', color: 'rgba(255,255,255,0.5)' }}>Cancel</button>
                    <button type="button" disabled={actionBusy} onClick={() => submitJobAction({ action: 'mark_complete', note: completeNote })} className="rounded-full px-3 py-1.5 text-[11px] disabled:opacity-40" style={{ background: '#2f7fb8', color: 'white' }}>{actionBusy ? 'Closing...' : 'Mark Complete'}</button>
                  </div>
                </div>
              )}
              ...
              {actionMessage && <div className="rounded-2xl px-3 py-2 text-[11px]" style={{ background: 'rgba(95,184,224,0.08)', border: '1px solid rgba(95,184,224,0.18)', color: 'rgba(255,255,255,0.72)' }}>{actionMessage}</div>}
            </div>
          </Section>
