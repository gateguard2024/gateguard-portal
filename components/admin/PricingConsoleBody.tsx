...
// PricingConsoleBody — the Pricing Console's full working surface, shared by:
//   • /admin/settings/pricing (standalone portal page)
//   • the "Pricing Console" glass shell on the Nexus admin (Internal) surface
// All content sits on white cards so it reads correctly on both the light
// portal background and the dark Nexus glass shell (two-tone, tech-tool style).
// Carries its own toolbar (+ Line Item / Save) so no TopBar is required.
...
function AddItemModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg rounded-2xl border border-border bg-white p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button onClick={onClose} className="rounded-xl border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted">Cancel</button>
          <button
            onClick={create}
...
/* ─── Body ───────────────────────────────────────────────────────────────── */
export function PricingConsoleBody() {
  ...
  return (
    <div className="space-y-5">
      {/* Toolbar — the body owns its actions so it works in any shell */}
      <div className="flex items-center justify-between gap-2">
        <div className="grid flex-1 grid-cols-2 gap-2 lg:grid-cols-4">
          {[
            { label: 'Lines', value: counts.total, cls: 'text-foreground' },
            { label: 'Quotable', value: counts.quotable, cls: 'text-emerald-600' },
            { label: 'Review', value: counts.review, cls: 'text-amber-600' },
            { label: 'Blocked', value: counts.open, cls: 'text-red-600' },
          ].map(s => (
            <div key={s.label} className="rounded-2xl border border-border bg-card px-3 py-2">
              <span className={`text-lg font-bold ${s.cls}`}>{loading ? '—' : s.value}</span>
              <span className="ml-2 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{s.label}</span>
            </div>
          ))}
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-[#6B7EFF]/40 bg-white px-3 py-2 text-sm font-semibold text-[#6B7EFF] transition-colors hover:bg-[#6B7EFF]/10"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-[#6B7EFF] px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-[#5A6BEB] disabled:opacity-40"
          >
      ...
      {loading ? (
        <div className="rounded-2xl border border-border bg-card py-14 text-center text-sm text-muted-foreground">Loading pricing catalog…</div>
      ) : (
        <>
          {SECTIONS.map(sec => {
