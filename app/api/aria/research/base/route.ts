...
function looksLikeArea(q: string): boolean {
  ...
  if (/\b(units?|doors?)\b/.test(ql) && /\d/.test(ql)) return true
  ...
  if (/\b(properties|complexes|communities|listings|multifamily|buildings|mdus?|sites|portfolios?)\b/.test(ql)) return true
