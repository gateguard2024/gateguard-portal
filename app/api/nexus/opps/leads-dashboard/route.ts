...
export async function GET() {
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const fus: any[] = Array.isArray(fuRows) ? fuRows : []
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mapFu = (t: any) => ({ id: String(t.id), type: typeOf(t.title), title: t.title ?? 'Follow-up', lead: t.linked_label ?? null, due: t.due_date })
    const followupsToday = fus.filter(t => t.due_date <= todayIso).map(mapFu)
    const followupsWeek = fus.filter(t => t.due_date > todayIso).map(mapFu)
