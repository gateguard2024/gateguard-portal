...
const { Edit2, Camera, Cpu, Server, Hammer, Download } = require('lucide-react') as any;
...
export default function ProductsPage() {
  ...
  const [bulkStatus,          setBulkStatus]          = useState<Record<string, 'pending' | 'searching' | 'found' | 'not_found'>>({});
  const [bulkRunning,         setBulkRunning]         = useState(false);

  // Products for selected brand that have no manual
  const bulkCandidates = bulkBrand
    ? products.filter(p => p.brand === bulkBrand && !p.manualUrl)
    : [];

  const bulkFoundCount = Object.values(bulkStatus).filter(s => s === 'found').length;
  const bulkTotalRun   = Object.values(bulkStatus).filter(s => s !== 'pending').length;
  const bulkTotalSelected = bulkSelected.size;
  ...
  const runBulkImport = async () => {
    ...
    for (const id of ids) {
      ...
      setBulkStatus(prev => ({ ...prev, [id]: 'searching' }));

      try {
        const res = await fetch('/api/kb/find-manual', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ product_id: product.id, product_name: product.name, brand: product.brand }),
        });
        const d = await res.json().catch(() => ({}));

        if (res.ok && (d.found || d.already)) {
          setBulkStatus(prev => ({ ...prev, [id]: 'found' }));
          if (d.manualUrl) {
            setProducts(prev => prev.map(p => p.id === id ? { ...p, manualUrl: d.manualUrl } : p));
          }
        } else {
          setBulkStatus(prev => ({ ...prev, [id]: 'not_found' }));
        }
      } catch {
        setBulkStatus(prev => ({ ...prev, [id]: 'not_found' }));
      }

      // 500ms delay between calls — rate limit friendly
  ...
  return (
    <div className="flex flex-col min-h-full">
      <TopBar title="Product Catalog" actions={
        <>
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg border border-violet-300 bg-violet-50 text-violet-700 text-sm font-medium hover:bg-violet-100 transition-colors"
          >
            <Download size={14}/> Bulk Import Manuals
          </button>
      ...
      {/* ── Bulk Import Manuals slide-over ──────────────────────────────────── */}
      {bulkImportOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          <div className="fixed inset-0 bg-black/40" onClick={() => { if (!bulkRunning) setBulkImportOpen(false); }} />
          <div className="relative w-[560px] h-full bg-white shadow-xl flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  Select a manufacturer, pick products, then run Find Online on all of them at once.
                </p>
              </div>
              <button
                onClick={() => { if (!bulkRunning) setBulkImportOpen(false); }}
                disabled={bulkRunning}
                className="p-1.5 rounded-lg hover:bg-slate-100 text-muted-foreground disabled:opacity-40"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              {/* Step 1 — Manufacturer grid */}
              <div>
                <p className="text-xs font-semibold text-foreground mb-2 uppercase tracking-wide">1. Select Manufacturer</p>
                <div className="grid grid-cols-3 gap-2">
                  {Object.entries(BRAND_COLORS).map(([brand, color]) => {
                    const count = products.filter(p => p.brand === brand && !p.manualUrl).length;
                    const isSelected = bulkBrand === brand;
                    return (
                      <button
                        key={brand}
                        onClick={() => {
                        ...
                        disabled={bulkRunning}
                        className={cn(
                          "flex flex-col items-center justify-center gap-1.5 rounded-xl border px-3 py-3 text-center text-xs font-medium transition-all disabled:opacity-40",
                          isSelected
                            ? "border-violet-500 bg-violet-50 text-violet-800 ring-2 ring-violet-400/30"
                      ...
                      >
                        ...
                        <span className="leading-tight line-clamp-2">{brand}</span>
                        {count > 0
                          ? <span className="text-[10px] text-violet-600 font-semibold">{count} missing</span>
              ...
              {bulkBrand && (
                <div>
                    ...
                    <p className="text-xs font-semibold text-foreground uppercase tracking-wide">
                      ...
                      {bulkCandidates.length > 0 && (
                        <span className="ml-2 font-normal text-muted-foreground normal-case tracking-normal">
                          ({bulkCandidates.length} without a manual)
                        </span>
                      )}
                    </p>
                    {bulkCandidates.length > 0 && (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => setBulkSelected(new Set(bulkCandidates.map(p => p.id)))}
                          disabled={bulkRunning}
                  ...
                  {bulkCandidates.length === 0 ? (
                    <div className="flex items-center gap-2 px-4 py-6 bg-emerald-50 border border-emerald-200 rounded-xl text-sm text-emerald-700 justify-center">
                      <CheckCircle2 size={16} />
                      All {bulkBrand} products already have manuals indexed.
                    </div>
                  ) : (
                    <div className="border border-border rounded-xl overflow-hidden divide-y divide-border">
                      {bulkCandidates.map(product => {
                        const status = bulkStatus[product.id];
                        const isChecked = bulkSelected.has(product.id);
                        return (
                          <label
                            key={product.id}
                            className={cn(
                              ...
                              bulkRunning && "cursor-default"
                            )}
                          >
                            <input
                              type="checkbox"
                              checked={isChecked}
                              disabled={bulkRunning || !!status}
                              onChange={e => {
                              ...
                              className="w-4 h-4 rounded border-border text-violet-600 accent-violet-600 shrink-0"
                            />
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-medium text-foreground truncate">{product.name}</p>
                              <p className="text-[10px] text-muted-foreground font-mono">{product.sku} · {product.category}</p>
                            </div>
                            {/* Status indicator */}
                            {status === 'searching' && (
                            ...
                            {status === 'pending' && !bulkRunning && (
                              <span className="text-[10px] text-slate-400 font-medium shrink-0">Pending</span>
                            )}
                            {status === 'pending' && bulkRunning && (
                              <span className="text-[10px] text-slate-400 font-medium shrink-0">Queued</span>
                            )}
                          </label>
                        );
                      })}
              ...
              {/* Progress bar — shown while running or after */}
              {bulkTotalSelected > 0 && bulkTotalRun > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[11px] text-muted-foreground font-medium">
                      {bulkRunning ? `Searching… ${bulkTotalRun} of ${bulkTotalSelected}` : `Complete — ${bulkTotalRun} of ${bulkTotalSelected} processed`}
                    </span>
                    <span className="text-[11px] font-semibold text-emerald-600">{bulkFoundCount} found</span>
                  </div>
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-violet-500 rounded-full transition-all duration-300"
                      style={{ width: `${Math.round((bulkTotalRun / bulkTotalSelected) * 100)}%` }}
                    />
                  </div>
                </div>
              )}
              ...
              {/* Done summary */}
              {!bulkRunning && bulkFoundCount > 0 && bulkTotalRun === bulkTotalSelected && bulkTotalSelected > 0 && (
                <div className="flex items-center gap-2 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl text-sm text-emerald-700">
                  <CheckCircle2 size={15} />
                  <span><strong>✅ {bulkFoundCount} manual{bulkFoundCount !== 1 ? 's' : ''} found</strong> and indexed. Manual dots updated in the product list.</span>
                </div>
              )}
            </div>

            {/* Footer */}
            <div className="shrink-0 px-6 py-4 border-t border-border bg-slate-50/50 flex items-center justify-between gap-3">
              <button
                onClick={() => setBulkImportOpen(false)}
                disabled={bulkRunning}
                className="px-4 py-2 rounded-xl border border-border text-sm text-muted-foreground hover:bg-slate-100 transition-colors disabled:opacity-40"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2 rounded-xl bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700 disabled:opacity-40 transition-colors"
              >
                {bulkRunning
                  ? <><Loader2 size={13} className="animate-spin" /> Searching…</>
                  : <><Download size={13} /> Find {bulkSelected.size > 0 ? `${bulkSelected.size} Manual${bulkSelected.size !== 1 ? 's' : ''}` : 'Manuals'}</>
                }
              </button>
            </div>
          </div>
