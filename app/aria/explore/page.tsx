...
export default function AriaExplorePage() {
  ...
  }, [items, view, initMap, fMinUnits, fGate, fBulk, fNew])
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 flex items-center gap-2 rounded-xl px-3 py-2.5 max-w-2xl" style={{ background: 'rgba(15,24,48,0.9)', border: '1px solid rgba(107,126,255,0.2)' }}>
            ...
            <input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && runSearch()}
              placeholder={CATEGORIES.find(c => c.key === category)?.hint}
          ...
          <button onClick={runSearch} disabled={loading || !query.trim()}
            ...
            {loading ? <Loader2 size={14} className="animate-spin" /> : <Search size={14} />}{loading ? 'Finding…' : 'Find'}
          </button>
          {/* List / Map toggle */}
          {items.length > 0 && (
