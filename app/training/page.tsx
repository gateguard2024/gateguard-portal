...
export default function TrainingPage() {
  ...
  async function toggleChapter(courseId: string, chapterId: string, completed: boolean) {
    // Fire confetti only when marking complete for the first time
    const alreadyDone = !!(progress[courseId]?.[chapterId])
    if (completed && !alreadyDone) {
      void fireConfetti('milestone')
    }
    setProgress(p => ({
      ...p,
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {certs.map(cert => (
              <div key={cert.courseId} className="bg-[#6B7EFF]/5 border border-[#6B7EFF]/20 rounded-xl p-4 flex flex-col gap-2">
                ...
                <a
                  href={`https://www.linkedin.com/sharing/share-offsite/?url=https%3A%2F%2Fportal.gateguard.co%2Ftraining&title=I%20earned%20the%20${encodeURIComponent(cert.courseTitle)}%20certificate%20from%20GateGuard`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors"
                  style={{ borderColor: '#0A66C2', color: '#0A66C2' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = '#EBF3FB'; }}
